...
import static com.example.colorplayer.utils.YoutubeUtil.convertDurationString;
import static com.example.colorplayer.utils.YoutubeUtil.convertViewCount;
...
public class YoutubeSearchHelper {
    ...
    private static final String URL_BACK_PART = "&type=video&maxResults=" + NUM_OF_VIDEOS_IN_ONE_REQUEST + "&order=relevance&key=" + API_KEY;
        ...
        protected ArrayList<YoutubeVideoItem> doInBackground(URL... urls) {
            ...
            try {
                ...
                for (int i = 0; i < a1.length(); i++) {
                    ...
                    String thumbnailUrl = o2.getJSONObject("snippet").getJSONObject("thumbnails").getJSONObject("high").getString("url");
                    YoutubeVideoItem video = new YoutubeVideoItem(title, videoId, thumbnailUrl, "0", "0:00");
                    videos.add(video);
                }
    ...
    private class GetLikesTask extends AsyncTask<ArrayList<YoutubeVideoItem>, Void, ArrayList<YoutubeVideoItem>> {
        ...
        protected ArrayList<YoutubeVideoItem> doInBackground(ArrayList<YoutubeVideoItem>... arrayLists) {
            ...
            for (int i = 0; i < videos.size(); i++) {
                ...
                String temp = getVideoInfo(v.videoId);
                v.viewCount = temp.split(",")[0];
                v.duration = temp.split(",")[1];
                videos.set(i, v);
            }
            return videos;
        }
        public String getVideoInfo(String videoId) {
            String urlString = "https://www.googleapis.com/youtube/v3/videos?part=statistics,contentDetails&id=" + videoId + "&key=" + API_KEY;
            String viewCount = "0";
            ...
            try {
                ...
                try {
                    ...
                    viewCount = o2.getJSONObject("statistics").getString("viewCount");
                    viewCount = convertViewCount(viewCount);

                    duration = o2.getJSONObject("contentDetails").getString("duration");
                    duration = convertDurationString(duration);
                    result = viewCount + "," + duration;
